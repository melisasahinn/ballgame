...
public class BallController : MonoBehaviour
{
    ...
    [SerializeField] private float _horizantalSpeed;
    [SerializeField] private float _horizantalLimit;
    ...
    private void HorizontalBallMove()
    {
        ...
        {
            _horizantal = Input.GetAxisRaw("Mouse X");
        }
        else
        {
            _horizantal = 0;
        }
        _newX = transform.position.x + _horizantal * _horizantalSpeed * Time.deltaTime;
        _newX = Mathf.Clamp(_newX, -_horizantalLimit, _horizantalLimit);
